...
def intfast(conn, min_cal, max_cal, pref, med_hist):
    """
...
def main(user_calorieintake):
    ...
    with conn:
        ...
        if age >= 18 and med_hist==0:
            meals = intfast(conn,min_cal,max_cal,pref,med_hist)
        else:
            meals = keto.keto(conn,min_cal,max_cal,pref,med_hist)
        print("************Done Executing Intermittent Fasting script! returning the plan\n")
        return meals
