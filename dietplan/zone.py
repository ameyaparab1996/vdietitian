...
def zone(conn, min_cal, max_cal, pref, med_hist):
    """
    ...
    if med_hist == 1:
        pre = Diff(pre, cholesterol)
    if med_hist == 2:
        pre = Diff(pre, diabetes)
    if med_hist == 3:
        pre = Diff(pre, hypothyroidism)
    cur = conn.cursor()
    i=1
    while i<4:
        print ("\nPlan ",i)
    ...
    while i<4:
        ...
        meals[meal_type][planno] = {"item1":veggies,"item1Quantity":weight1,
                                    ...
                                    "item3Cals":cal3,"item3Proteins":protein3,
                                    "item3Carbs":carb3,"item3Fats":fat3}
...
def main(user_calorieintake):
    ...
    with conn:
        ...
        min_cal = user_calorieintake #3005
        max_cal = 2995
        age = 21
        pref = 0 #0 - Non Veg, 1 - Veg, 2 - Egg Veg
        med_hist = 1 #0 - None, 1 - Cholesterol, 2 - Diabetes, 3 - Hypothyroidism
        meals = zone(conn,min_cal,max_cal,pref,med_hist)
        print("************Done Executing Zone diet script! returning the plan\n")
        return meals
